...
async fn async_test_download_server() {
    ...
    substrate_core::download::server::download_server::download_server(
        config,
        true,
        None,
        dir.path(),
    ...
    let run_script = dir.path().join("servers").join("test").join("run.sh");

    assert!(run_script.exists());

    let eula = dir.path().join("servers").join("test").join("eula.txt");

    let content = tokio::fs::read_to_string(eula).await.unwrap();
